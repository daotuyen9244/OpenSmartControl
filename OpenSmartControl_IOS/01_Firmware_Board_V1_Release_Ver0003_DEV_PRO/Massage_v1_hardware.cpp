    ...
    case HOME_IDLE:
      {
        ...
        mySerial.println(sensorUpActive ? "ACTIVE" : "inactive");
        mySerial.print("  LMT_DOWN: ");
        mySerial.println(sensorDownActive ? "ACTIVE" : "inactive");
...
void startAutoMode() {
  ...
  mySerial.print("Start tick: ");
  mySerial.println(getAutoModeStartTick());
  mySerial.print("autoLastDirChangeTick reset to: ");
  mySerial.println(getAutoLastDirChangeTick());
...
void stopAutoMode() {
  ...
  // ═══════════════════════════════════════════════
  // TRIGGER GO HOME SEQUENCE
  // ═══════════════════════════════════════════════
  // Set homeRun to FALSE to trigger GO HOME sequence
  // This will move the chair back to home position (2s down from sensor UP)
  setHomeRun(false);
  mySerial.println("  - GO HOME sequence triggered");
  mySerial.println("  - Chair will return to home position");
...
void processAuto() {
  ...
  if (detectedProgram != getCurrentAutoProgram()) {
    ...
    mySerial.println(rollSpotMode ? "ON" : "OFF");
    mySerial.println();
    mySerial.print("Switch count: ");
    mySerial.println(programSwitchCount);
    mySerial.println("==============================");
...
void printSystemStatus() {
  ...
  mySerial.print("allowRun:         ");
  mySerial.println(getAllowRun() ? "TRUE" : "FALSE");
  mySerial.print("modeAuto:         ");
  mySerial.println(getModeAuto() ? "TRUE" : "FALSE");
  mySerial.print("homeRun:          ");
  mySerial.println(getHomeRun() ? "TRUE" : "FALSE");
  mySerial.println("--- MODE FLAGS ---");
  mySerial.print("autodefaultMode:  ");
  mySerial.println(getAutodefaultMode() ? "TRUE" : "FALSE");
  mySerial.print("rollSpotMode:     ");
  mySerial.println(getRollSpotMode() ? "TRUE" : "FALSE");
  mySerial.print("kneadingMode:     ");
  mySerial.println(getKneadingMode() ? "TRUE" : "FALSE");
  mySerial.print("compressionMode:  ");
  mySerial.println(getCompressionMode() ? "TRUE" : "FALSE");
  mySerial.print("percussionMode:   ");
  mySerial.println(getPercussionMode() ? "TRUE" : "FALSE");
  mySerial.print("combineMode:      ");
  mySerial.println(getCombineMode() ? "TRUE" : "FALSE");
  mySerial.println("--- FEATURES ---");
  mySerial.print("intensityLevel:   ");
  mySerial.println(intensityLevel);
  mySerial.print("useHighPrecisionTimer: ");
  mySerial.println(useHighPrecisionTimer ? "TRUE (1ms)" : "FALSE (10ms)");
  mySerial.println("------------------");
  mySerial.print("manualPriority:   ");
  mySerial.println(manualPriority ? "TRUE" : "FALSE");
  mySerial.print("RL1running:       ");
  mySerial.println(RL1running ? "TRUE" : "FALSE");
  mySerial.print("RL2running:       ");
  mySerial.println(RL2running ? "TRUE" : "FALSE");
  mySerial.print("Timer ticks (10ms): ");
  mySerial.println(timerTicks);
  mySerial.print("Timer ticks (1ms):  ");
  mySerial.println(timerTicks1ms);
  mySerial.println("--- AUTO TOTAL TIMER (20min) ---");
  mySerial.print("autoTotalTimerActive: ");
  mySerial.println(getAutoTotalTimerActive() ? "TRUE" : "FALSE");
  mySerial.print("autoTotalTimerExpired: ");
  mySerial.println(getAutoTotalTimerExpired() ? "TRUE" : "FALSE");
  if (getAutoTotalTimerActive() && !getAutoTotalTimerExpired()) {
    ...
    mySerial.println(" ticks");
    mySerial.print("Remaining: ");
    mySerial.print(remaining / 6000);
    mySerial.print("m ");
    mySerial.print((remaining % 6000) / 100);
    mySerial.println("s");
  }
  mySerial.println("====================");
}
