# 📱 QR Code Format - Massage Chair Control App
...
**Cách hoạt động:**
1. User quét QR → App tìm device có tên "MASSAGE_DEVICE"
...
void setup() {
  ...
  Serial.println("BLE device ready: MASSAGE_DEVICE");
}
...
4. App sẽ tìm device "MASSAGE_DEVICE" và kết nối
...
### **Ghế massage A1:**
```json
{"type":"massage_device","name":"MASSAGE_DEVICE_A1"}
```

### **Ghế massage B2:**
```json
{"type":"massage_device","name":"MASSAGE_DEVICE_B2"}
```
...
```cpp
// ESP32 Code
BLEDevice::init("MASSAGE_DEVICE");  // ← Phải khớp với QR
```

```json
// QR Code
{"type":"massage_device","name":"MASSAGE_DEVICE"}  // ← Phải khớp với ESP32
```
...
- `MASSAGE_DEVICE` = `massage_device` = `Massage_Device` ✅

---

## 🐛 **Troubleshooting**

### **Lỗi: "Device MASSAGE_DEVICE not found"**
...
**Giải pháp:**
1. Check tên BLE: `BLEDevice::init("MASSAGE_DEVICE")`
...
**Giải pháp:**
1. Check JSON format: `{"type":"massage_device","name":"MASSAGE_DEVICE"}`
...
```json
{"type":"massage_device","name":"MASSAGE_DEVICE"}
```
